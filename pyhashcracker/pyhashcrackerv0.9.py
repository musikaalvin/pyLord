            ...
            try:
                if algo_name == 'bcrypt':
                    ...
                    for _ in range(iterations):
                        self.check_bcrypt(test_password, dummy_hash)
                elif algo_name == 'pbkdf2_sha256':
                    dummy_hash = pbkdf2_sha256.hash(test_password)
                    for _ in range(iterations):
                        self.check_pbkdf2(test_password, dummy_hash)
                elif algo_name == 'argon2':
                    dummy_hash = argon2.hash(test_password)
                    for _ in range(iterations):
                        self.check_argon2(test_password, dummy_hash)
                else:
                    dummy_hash = hashlib.new(algo_name, test_password.encode()).hexdigest()
                    for _ in range(iterations):
                        self.check_hash(test_password, dummy_hash, algo_name)

                end_time = time.time()
                hps = iterations / (end_time - start_time) if (end_time - start_time) > 0 else 0
...
class Hashcracker(tk.Frame):
    def __init__(self, master):
        ...
        self.running = False
        self.paused = threading.Event()  # Used for pause/resume control
    ...
    def configure_ui(self):
        ...
        self.style.configure('TLabel', background='#282C34', foreground='white',
                             font=('Courier', 10))
        self.style.configure('TButton', background='#3a3f4b',
        ...
        self.style.configure('TCombobox', fieldbackground='#333333',
                             foreground='white', selectbackground='#4a4f5b')
        self.style.configure('TCheckbutton', background='#282C34',
                             foreground='white', font=('Courier', 10))
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="PyHashCracker Pro", font=("Courier New",
                 18, "bold"), bg="#282C34", fg="#00FF00").pack()
        tk.Label(header_frame, text="by pyLord@cyb3rh4ck3r04", font=("Courier",
                 10), bg="#282C34", fg="#00FF00").pack()
        ...
        self.console = scrolledtext.ScrolledText(log_frame, height=20, width=80,
                                                 bg="#1e1e1e", fg="cyan", wrap=tk.WORD, font=("Courier", 9))
        self.console.pack(expand=True, fill='both')
        self.console_log("Welcome to PyHashCracker Pro. Select an attack mode and provide inputs.")

        # Result Label
        self.result_label = ttk.Label(self.master, text="", font=("Courier", 14,
                                       "bold"), foreground='lime')
        ...
        ttk.Label(hash_frame, text="Target Hash:".ljust(15), font=('Courier',
                  10, 'bold')).pack(side=tk.LEFT)
        self.hash_entry = ttk.Entry(hash_frame)
        self.hash_entry.pack(side=tk.LEFT, expand=True, fill='x', padx=5)
        self.hash_type_label = ttk.Label(hash_frame, text="Type: (auto-detect)",
                                        width=20)
        ...
        ttk.Label(file_frame, text="Wordlist File:".ljust(15), font=('Courier',
                  10, 'bold')).pack(side=tk.LEFT)
        ...
        ttk.Label(mode_frame, text="Attack Mode:".ljust(15), font=('Courier',
                  10, 'bold')).pack(side=tk.LEFT)
        self.mode_selector = ttk.Combobox(mode_frame, values=['Dictionary',
                                  'Normal Attack', 'Brute-force', 'Combo', 'Mask',
                                  'Hybrid', 'Rainbow Table'],
                                  state='readonly', width=15)
        self.mode_selector.pack(side=tk.LEFT, padx=5)
        self.mode_selector.set('Normal Attack')

        self.mask_label = ttk.Label(mode_frame, text="Mask Pattern:",
        ...
        ttk.Label(brute_frame, text="Brute-force:".ljust(15), font=('Courier',
                  10, 'bold')).pack(side=tk.LEFT)
        ...
        ttk.Button(adv_frame, text="Load Rules",
                   command=self.load_rules).pack(side=tk.LEFT, padx=5)
        ttk.Button(adv_frame, text="Generate Rainbow DB",
                   command=self.generate_rainbow_table_popup).pack(side=tk.LEFT, padx=5)
        ttk.Button(adv_frame, text="Benchmark",
                   command=self.run_benchmark).pack(side=tk.LEFT, padx=5)
        self.gpu_var = tk.BooleanVar(value=False)
        gpu_check = ttk.Checkbutton(adv_frame, text="GPU Acceleration (Not Implemented)",
                                    variable=self.gpu_var, state='disabled')
        ...
        self.progress_bar = ttk.Progressbar(status_area, orient=tk.HORIZONTAL,
        ...
        self.attempts_label = ttk.Label(info_frame, text="Attempts: 0 / 0",
                                        width=30)
        self.attempts_label.pack(side=tk.LEFT, expand=True, fill='x')
        self.speed_label = ttk.Label(info_frame, text="Speed: 0 p/s",
                                     width=20)
        self.speed_label.pack(side=tk.LEFT, expand=True, fill='x')
        self.elapsed_label = ttk.Label(info_frame, text="Elapsed: 00:00:00",
                                       width=20)
        self.elapsed_label.pack(side=tk.LEFT, expand=True, fill='x')
        self.remaining_label = ttk.Label(info_frame, text="Remaining: --:--:--",
                                         width=20)
        ...
        self.toggle_btn = tk.Button(btn_frame, text="▶ START CRACKING",
                                    command=self.toggle_cracking,
                                    bg="#008000", fg="white",
    ...
    def toggle_pause(self):
        if not self.running:
            return
        # Pause/resume logic: Event set = running, clear = paused
    ...
    def validate_inputs(self, mode):
        ...
        if self.algo == 'unknown':
            messagebox.showwarning("Input Error",
                                   "Could not determine hash type. Cracking may fail.")
        ...
        if mode == 'Mask' or mode == 'Hybrid':
            ...
            if not mask or mask == 'e.g. Pass?d?d?d':
                messagebox.showerror("Input Error", "A valid mask pattern is required for this mode.")
                return False
    ...
    def start_cracking(self):
        ...
        self.running = True
        self.paused.set()  # Allow thread to run
        ...
        if mode == 'Dictionary':
            ...
            self.wordlist2_path = filedialog.askopenfilename(
                title="Select SECOND Wordlist for Combo Attack")
            if self.wordlist2_path and os.path.exists(self.wordlist2_path):
                target_func = self._combo_attack
            else:
                self.console_log("ERROR: Second wordlist for Combo attack not selected or not found.")
                self.stop_cracking()
                return
        elif mode == 'Hybrid':
            target_func = self._hybrid_attack
        else:
            self.console_log(f"ERROR: Unknown attack mode '{mode}'.")
            self.stop_cracking()
            return
    ...
    def stop_cracking(self, reason=""):
        self.running = False
        self.paused.set()  # Unblock thread if paused
        ...
        self.pause_btn.config(state='disabled', text="❚❚ PAUSE", bg="#FFA500")
        self.progress_bar['value'] = 0

        if "FOUND" not in reason.upper():
    ...
    def _dictionary_attack(self):
        try:
            with open(self.wordlist1_path, 'r', errors='ignore') as f:
                # No total calculation for dictionary mode (progress is indeterminate)
                self.total_passwords = 0
                for line in f:
    ...
    def _normal_attack(self):  # Wordlist + Rules
        if not self.rules:
            ...
            self.console_log("No rules loaded. Running a standard dictionary attack.")
            self._dictionary_attack()
            return
        try:
            with open(self.wordlist1_path, 'r', errors='ignore') as f:
                words = [line.strip() for line in f if line.strip()]
            # Calculate total number of variants (exact count)
            total = 0
            for word in words:
                total += len(self.core.apply_rules(word, self.rules))
    ...
    def _hybrid_attack(self):
        ...
        try:
            ...
            mask_candidates = list(self.core.generate_from_mask(mask))
            mask_cardinality = len(mask_candidates)
            ...
            for word in words:
                if not self.running: break
                # Append mask
                for suffix in mask_candidates:
                    if not self.running: break
                    self.paused.wait()
                    self.check_and_report(word + suffix)
                if not self.running: break
                # Prepend mask
                for prefix in mask_candidates:
                    if not self.running: break
                    self.paused.wait()
    ...
    def check_and_report(self, password):
    ...
    def update_status_labels(self):
        ...
        if now - self.last_update_time >= 1:  # Update speed every second
            ...
            speed = processed_since_last / time_delta if time_delta > 0 else 0
            ...
            # Update ETR
            if speed > 0 and self.total_passwords > 0 and self.total_passwords != float('inf'):
        ...
        if self.total_passwords > 0 and self.total_passwords != float('inf'):
    ...
    def generate_rainbow_table_popup(self):
        ...
        ttk.Label(popup, text="Select Algorithm for the Table:",
                  background="#282C34", foreground="white").pack(pady=5)

        # Only allow fast, unsalted hashes for rainbow tables
        allowed_algos = ['md5', 'sha1', 'sha256', 'sha512', 'ntlm', 'md4', 'sha3_256']

        algo_var = tk.StringVar()
        algo_box = ttk.Combobox(popup, textvariable=algo_var,
                                values=allowed_algos, state='readonly')
                ...
                with open(wordlist_path, 'r', errors='ignore') as f:
                    for line in f:
                        ...
                        if algo == 'ntlm':
                            ...
                            hashed_value = hashlib.new(algo, password.encode()).hexdigest()

                        cursor.execute("INSERT OR IGNORE INTO hashes (password, hash) VALUES (?, ?)",
    ...
    def run_benchmark(self):
        # Use queue for thread-safe UI logging
        self.core.console_log = lambda msg: self.queue_update(('log', msg))
